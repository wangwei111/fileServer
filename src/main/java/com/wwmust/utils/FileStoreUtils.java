...
public class FileStoreUtils {
        ...
        if (folder.isDirectory()) {
            ...
            for (File i : files) {
                if (i.isDirectory()) {
                    ...
                    FileStoreUtils.deleteFolder(i.toString());
                    i.delete();
                }
                //删除文件
                if (i.isFile()) {
                    FileStoreUtils.deleteFile(i.toString());
                }
            }
        }
        //如果传入的是文件名，不让删除
    ...
    public static boolean deleteFolderOrFile(String folderNameOrFileName){
        ...
        if (file.isDirectory()) {
            FileStoreUtils.deleteFolder(folderNameOrFileName);
        }
        if (file.isFile()) {
            FileStoreUtils.deleteFile(folderNameOrFileName);
        }
        return true;
    }
